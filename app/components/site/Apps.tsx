...
function AppIcon({ app }: { app: AppConfig }) {
  ...
  return (
    <div
      className={`flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl shadow-sm ${t.iconBg}`}
      aria-hidden
    >
      <Icon className={`h-7 w-7 ${t.iconFg}`} strokeWidth={2.25} />
    </div>
  );
}
  ...
  return (
    <article className="reveal flex h-full flex-col overflow-hidden rounded-3xl border border-border bg-card p-6 shadow-soft grain hover-lift md:p-7">
      ...
      <a
        ...
        data-cursor="install"
        className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-ink px-5 py-3.5 text-[15px] font-semibold text-background transition hover:scale-[1.02] hover:bg-ink/90"
      >
        {app.cta}
